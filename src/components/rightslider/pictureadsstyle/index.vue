        ...
        >
          <template #item="{ element, index }">
            <section class="imgBanner">
              ...
              <!-- 图片 -->
              <div class="imag">
                <img :src="element.src" :alt="index" draggable="false" />
              </div>
              <!-- 标题和链接 -->
              <div class="imgText">
                <el-input
...
type PictureadsStyleProps = Record<string, any>;
const props = defineProps({
  datas: Object as PropType<PictureadsStyleProps>,
});
...
const styleText = computed(() => {
  let data;
  if (props.datas?.swiperType === 0) data = "一行一个";
  if (props.datas?.swiperType === 1) data = "轮播海报";
  if (props.datas?.swiperType === 2) data = "多图单行";
  if (props.datas?.swiperType === 3) data = "立体模式";
  if (props.datas?.swiperType === 4) data = "导航横向滑动";
