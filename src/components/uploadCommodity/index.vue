<template>
  <div class="uploadCommodity">
    ...
    >
      ...
      >
        <el-option
          v-for="item in datas.optionsType"
          :key="item.name"
          :label="item.name"
          :value="item.type"
        ></el-option>
      ...
      <el-select
        ...
        v-if="datas.type !== '11'"
        v-model="datas.dialogImageUrl.name"
        placeholder="请选择图片跳转链接"
        size="small"
        @change="changeId"
        :no-data-text="datas.emptyText"
        @visible-change="
          (isVisible: any) => {
...
const emits = defineEmits(["uploadListInformation"]);
// 数据集合
const datas = reactive({
  dialogVisible: false, //弹框默认隐藏
  dialogImageUrl: {} as any, // 选择的数据
  type: "2",
...
// 选择类型
const changeType = (isVisible: any, linkType: any) => {
...
const uploadInformation = () => {
  const disLogObj: any = datas.dialogImageUrl;
  disLogObj.httpType = datas.type;
  emits("uploadListInformation", datas.dialogImageUrl);
