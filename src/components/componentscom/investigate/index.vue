      ...
      <div v-if="item1.type == 1" class="xiala">
        ...
        <div class="select">
          <input
            ...
            class="readinput"
            @click="showpic($event, index1.toExponential)"
            :value="item1.value2"
          />
...
//点击显示下拉框
const showpic = (e: Event, index1: any) => {
  e.stopPropagation();
  props.datas?.jsonData.forEach((el: any) => {
    el.showPicker = false;
  });
  props.datas!.jsonData[index1].showPicker =
    !props.datas?.jsonData[index1].showPicker;
};

// //下拉选择
const xuanze = (index1: string | number) => {
  props.datas!.jsonData[index1].showPicker = false;
};
