        ...
        >
          ...
          <h5
            :style="{
              ...
              'text-align': datas?.positions === 'center' ? 'center' : 'left',
            }"
          >
            这里显示商品名称，最多显示1行
          </h5>
    ...
    <div class="tab-type2">
      ...
      >
        ...
        >
              ...
              >
                ...
                <h5
                  :style="{
                    'font-weight': datas?.textWeight,
                    'text-align':
                      datas?.positions === 'center' ? 'center' : 'left',
                  }"
                >
                  这里显示商品名称，最多显示1行
                </h5>
          ...
          >
            <div
              v-for="(itemC, index) in item.imageList"
              :key="index"
              class="defaultcommodityList"
              :class="[
                datas.commodityType === 0 ? 'defaultcommodityList0' : '',
...
const active1 = ref(0);
const active = ref(0);
const listSwitchName = ref("");
const tagColor = ref("#07c160");
let styleString = reactive<Partial<FordProps>>({});

onMounted(() => {
  nextTick(() => {
    if (listSwitchName.value) {
      const queryObj: any = listSwitchName.value[0];
      queryObj.querySelector(".van-sidebar-item--select").style.borderColor =
        props.datas?.tabColor;
    }

    listSwitchName.value =
...
const tagPositionStyle = () => {
  ...
  /* '左上',
        ...
        '又上',
        '又下' */
  switch (props.datas?.tagPosition) {
    case 0: // 左上
      styleString.top = "5px";
      styleString.left = "0px";
      break;
    case 1: // 左下
      styleString.bottom = "5px";
      styleString.left = "0px";
      break;
    case 2: // 右上
      styleString.top = "5px";
      styleString.right = "0px";
      props.datas?.commoditycornertype === 0
        ? (styleString.borderRadius = "10px 0px 0px 10px !important")
        : "";
      break;
    case 3: // 右下
      styleString.bottom = "5px";
...
watch(
  () => props.datas?.tabColor,
  () => {
    const queryObj: any = listSwitchName.value;
    queryObj.querySelector(".van-tabs__line").style.backgroundColor =
      props.datas?.tabColor;
    queryObj.querySelector(".van-sidebar-item--select").style.borderColor =
      props.datas?.tabColor;
  }
...
watch(
  () => props.datas?.commoditylisttype,
  () => {
    const queryObj: any = listSwitchName.value;
    if (props.datas?.commoditylisttype === 2) {
      queryObj.querySelector(".van-sidebar-item--select").style.borderColor =
        props.datas?.tabColor;
    }
  }
...
watch(
  () => props.datas?.tagPosition,
  () => {
    tagPositionStyle();
  }
);
watch(
  () => props.datas?.commoditycornertype,
  () => {
    tagPositionStyle();
  }
);
